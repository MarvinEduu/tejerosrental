...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_property'])) {
    ...
    $update_query = $conn->prepare("UPDATE `properties_tb` SET name=?, address=?, city=?, state=?, zipCode=?, houseType=?, bedroomNum=?, bathroomNum=?, size=?, rentAmount=?, details=?, latitude=?, longitude=? WHERE propertyId=?");
    $result = $update_query->execute([$name, $address, $city, $state, $zipCode, $houseType, $bedroomNum, $bathroomNum, $size, $rentAmount, $details, $latitude, $longitude, $pid]);
...
<head>
    ...
    <style>
        .file-input {
            ...
            padding: 0.5rem 0.75rem; /* This is equivalent to Tailwind's px-3 py-2 */
...
<body class="bg-gray-100 font-family-karla flex">
    ...
    <div class="w-full h-screen overflow-x-hidden border-t flex flex-col">
        <main class="w-full flex-grow p-6">
            ...
            <form action="update-property-handler.php" method="POST" enctype="multipart/form-data" class="p-10 bg-white rounded shadow-xl">
            <input type="hidden" name="propertyId" value="<?= htmlspecialchars($property['propertyId']) ?>">
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    ...
                    <select class="w-full px-5 py-3 text-gray-700 bg-gray-200 rounded" id="houseType" name="houseType">
                        ...
                        <option value="Dorm" <?= $property['houseType'] === 'Dorm' ? 'selected' : '' ?>>Dorm</option>
                        <option value="Bedspace" <?= $property['houseType'] === 'Bedspace' ? 'selected' : '' ?>>Bedspace</option>
                    </select>
                    </div>
                    <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
        ...
        <button class="px-4 pt-3 pb-3 text-white font-light tracking-wider bg-blue-900 rounded" type="submit" name="update_property">Update Property</button>
        <button class="px-4 text-gray-900 font-light tracking-wider bg-gray-200 rounded" type="button" onclick="window.location.href='landholder-table.php'">Cancel</button>
    </div>
            </form>
        </main>
    </div>
