...
// Query to retrieve seller's properties, ensuring only active and unbooked properties are fetched
$propertiesQuery = $conn->prepare("SELECT * FROM `properties_tb` WHERE `landholder_id` = :landholder_id AND status != 'Pending' AND status != 'Rejected' AND is_deleted = 0 AND propertyId NOT IN (SELECT propertyId FROM `bookings_tb` WHERE status = 'Accepted' AND endDate > NOW())");
...
<body>
    ...
    <nav class="breadcrumbs flex items-center ml-6">
        <!-- Home -->
